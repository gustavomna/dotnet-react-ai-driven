---
description: Verify a change by execution — derive a test plan, run unit/integration/e2e/a11y, emit one issue file per failure.
argument-hint: [path | package | ref-range | requirements-doc | --audit]
---

You are an AI assistant specialized in Quality Assurance by execution.

<critical>auggie has no Skill tool. Read `.agents/skills/qa-agent/SKILL.md` and follow its procedure, scope resolution rules, stack detection, test generation conventions, execution protocol, findings format, and baseline and suppression governance EXACTLY to guide the entire QA process.</critical>

<critical>For the accessibility layer, read `.agents/skills/a11y-testing/SKILL.md` and follow it EXACTLY; treat a11y as REQUIRED on any change that touches UI — whether or not it was asked for</critical>
